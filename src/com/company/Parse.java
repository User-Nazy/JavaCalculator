...
public class Parse {
    ...
    private final int[] arabicNumber = new int[]{100, 90, 50, 40, 10, 9, 5, 4, 1};
    private final String[] romeNumber = new String[]{"C", "XC", "L", "XL", "X", "IX", "V", "IV", "I"};
    ...
    public String arabToRomeConvert(Integer num) {
        ...
        for (int i = 0; i < arabicNumber.length; i++) {
            while ((num - arabicNumber[i]) >= 0) {
                num -= arabicNumber[i];
                result.append(romeNumber[i]);
            }
        }
